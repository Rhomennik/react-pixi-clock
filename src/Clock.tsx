...
const Clock: FC<ClockProps> = ({ input }) => {
  ...
  const [hourPointerPosition, setHourPointerPosition] = useState({
    x: 0,
    y: 0,
  });
  const [minutePointerPosition, setMinutePointerPosition] = useState({
    x: 0,
    y: 0,
  });
  const [secondPointerPosition, setSecondPointerPosition] = useState({
    x: 0,
    y: 0,
  });
  ...
  console.log(hourPointerPosition);
  ...
  return (
    <Container>
      ...
      <Sprite
        ...
        onpointerdown={(e) => {
          e.stopPropagation();
          setHourPointerPosition({ x: e.data.global.x, y: e.data.global.y });
        }}
        pointermove={(e) => {
          const sprite = e.currentTarget as any;
          console.log("que pasa wei", sprite.dragging);
          if (sprite.dragging) {
            const newPosition = e.target.position.clone();
            newPosition.x += e.data.global.x - hourPointerPosition.x;
            newPosition.y += e.data.global.y - hourPointerPosition.y;
            e.target.position.set(newPosition.x, newPosition.y);
            setHourPointerPosition({ x: e.data.global.x, y: e.data.global.y });
          }
        }}
        onpointerup={() => {
          // Faça algo quando o arraste do ponteiro da hora terminar (por exemplo, atualizar o valor da hora selecionada)
        }}
      />
